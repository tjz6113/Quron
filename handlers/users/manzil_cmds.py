...
async def get_json(manzil_num):
    ...
    transliteration = req.get(f"http://api.alquran.cloud/v1/manzil/{manzil_num}/en.transliteration").json()['data']['ayahs']
    translation_en_arr = req.get(f"http://api.alquran.cloud/v1/manzil/{manzil_num}/en.asad").json()['data']['ayahs']
    translation_ru_arr = req.get(f"http://api.alquran.cloud/v1/manzil/{manzil_num}/ru.kuliev-alsaadi").json()['data']['ayahs']
...
async def manzil_callbacks(call: types.callback_query, state: FSMContext):
    ...
    if call.data == "next-ayahs-manzil":
        ...
        dig_n_manzil = {
            "uzl":"Boshqa manzil raqamini yuboring!",
            "uzk":"Бошқа манзил рақамини юборинг!",
            "ru":"Отправить другой номер манзили",
            "en":"Send another manzil number"
        }
...
async def make_msg(num, lang, arabic_arr, transliteration_arr, translation_uz_arr, translation_ru_arr, translation_en_arr):
...
async def send_messages(message: types.Message, state: FSMContext, manzil, lang):
    ...
    for i in range(st_ayah - 1, lt_ayah):
        audioURL, text, numberInSurah = await make_msg(i, lang, arabic_arr, transliteration_arr, translation_uz_arr, translation_ru_arr, translation_en_arr)
        if len(text) > 4096:
    ...
    if lt_ayah == len(arabic_arr):
        end_manzil = {
            "uzl":"Manzil oxiriga yetdingiz. Pastdagilardan birini tanlang!",
...
async def checkNumber(message: types.Message, state: FSMContext):
    ...
    if manzil > 7 or manzil < 1:
        en_num_limit = {
            "uzl": "Iltimos 1 va 7 orasida bo'lgan raqam kiriting!",
            "uzk": "Илтимос 1 ва 7 орасида бўлган рақам киритинг!",
            "ru": "Пожалуйста, введите число от 1 до 7!",
