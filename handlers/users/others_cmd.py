...
async def cancel_everything(message: Message, state: FSMContext):
    await state.finish()
    await message.answer("/start - Botni ishga tushuring. Запустить бота")
...
@dp.message_handler(state='*', commands=['getall_uzbek'])
async def getallUZ(message: Message):
    num = db_surah.count_surahs_uz()[0]
    await message.answer(str(num) + " ta sura kiritilgan!")
...
async def get_id_for_surah(message: Message, state: FSMContext):
    ...
    while str(await state.get_state()) == 'getting_file_ids_for:write_ar':
        try:
            db_surah.add_file_id_ar(surah_num=surah_num, file_id=file_id)
            await add_arabic_audios(message)
        except sqlite3.IntegrityError as err:
            await message.answer(err)

    while str(await state.get_state()) == "getting_file_ids_for:write_uz":
        try:
            db_surah.add_file_id_uz(surah_num=surah_num, file_id=file_id)
            await add_uzbek_audios(message)
        except sqlite3.IntegrityError as err:
            await message.answer(err)
